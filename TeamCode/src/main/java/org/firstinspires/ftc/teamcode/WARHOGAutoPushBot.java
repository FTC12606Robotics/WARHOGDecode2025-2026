...
public class WARHOGAutoPushBot extends OpMode {
    ...
    boolean useTargetHeading = true; //To decide if we are going to use heading target correction.
            ...
            case 2:
                ...
                if(!follower.isBusy()) {
                    ...
                    while (headingError != 0){
                        //Auto algin angle
                        if (useTargetHeading && useCamera){
                            //Find target heading and error
                            headingError = currentHeading - targetHeading;
                            if (abs(headingError) <= .04) {headingError =0 ;} //Allows small error range
                        }
                        else{
                            headingError = 0;
                        }

                        drivetrain.applyTurnCorrection(headingError, .5);
    ...
    public void init_loop() {
        ...
        if (currentGamepad1.right_stick_button && !previousGamepad1.right_stick_button) {
            useTargetHeading = !useTargetHeading;
        }
        ...
        telemetry.addData("Use Target Heading (rsbtn)", useTargetHeading);
    ...
    public void loop() {
        ...
        currentHeading = follower.getPose().getHeading()*180/PI;
        ...
        telemetry.addData("heading current", currentHeading);
